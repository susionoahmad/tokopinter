...
export default function App() {
  ...
  // Handle local persistence restore on boot
  useEffect(() => {
    const savedTenantId = localStorage.getItem('pos_active_tenant_id');
    const savedRole = localStorage.getItem('pos_active_role') as 'superadmin' | 'owner' | 'cashier' | null;
    const savedLockedState = localStorage.getItem('pos_is_locked') === 'true';

    if (savedTenantId) {
      const cachedTenantsRaw = localStorage.getItem('saas_cached_tenants');
      const cachedTenants: Tenant[] = cachedTenantsRaw ? JSON.parse(cachedTenantsRaw) : [];
      const match = cachedTenants.find(t => t.id === savedTenantId);
      if (match) {
